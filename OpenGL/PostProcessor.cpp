...
void PostProcessor::CreateBuffers()
{
	...
	// Create a renderbuffer object and stencil attachment (we won't be sampling these)
	...
	// Now that we created the framebuffer and added all attachment we want to check if it is complete
...
void PostProcessor::End()
{
	...
	m_postShader->SetTextureSampler("ScreeTexture", GL_TEXTURE0, 0, m_textureColorbuffer);
	BindVertices();
