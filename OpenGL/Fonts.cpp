...
void Fonts::Initialize(string _name, FT_UInt _size)
{
	...
	m_orthProj = glm::ortho(0.0f, (float)r.m_width, 0.0f, (float)r.m_height);
...
void Fonts::CreateCharacters()
{
	...
	for (unsigned char c = 0; c < 128; c++)
	{
		...
		//Now store character fpr later use
...
void Fonts::RenderText(std::string _text, float _x, float _y, float _scale, glm::vec3 _color)
{
	...
	glUniformMatrix4fv(glGetUniformLocation(m_shader->GetProgramID(), "Projection"), 1, GL_FALSE, glm::value_ptr(m_orthProj));
	glBindBuffer(GL_ARRAY_BUFFER, m_vertexBuffer);
